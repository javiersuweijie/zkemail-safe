...
template SubjectRegex (msg_bytes) {
    ...
    for (var i = 0; i < num_bytes; i++) {
        ...
        multi_or[2][i].in[7] <== eq[20][i].out;
        multi_or[2][i].in[8] <== eq[21][i].out;
        multi_or[2][i].in[9] <== eq[22][i].out;
        ...
        and[15][i].b <== eq[25][i].out;
        eq[26][i] = IsEqual();
        eq[26][i].in[0] <== in[i];
        eq[26][i].in[1] <== 10;
        and[16][i] = AND();
        and[16][i].a <== states[i][8];
        and[16][i].b <== eq[26][i].out;
        ...
        and[32][i].b <== eq[42][i].out;
        states[i+1][26] <== and[32][i].out;
        eq[43][i] = IsEqual();
        eq[43][i].in[0] <== in[i];
        eq[43][i].in[1] <== 32;
        and[33][i] = AND();
        and[33][i].a <== states[i][26];
        and[33][i].b <== eq[43][i].out;
    ...
    signal output reveal_proposal[msg_bytes];
    for (var i = 0; i < msg_bytes; i++) {
        reveal_proposal[i] <== in[i+1] * (states[i+2][1]);
    }

    signal output reveal_safe[msg_bytes];
    for (var i = 0; i < msg_bytes; i++) {
        reveal_safe[i] <== in[i+1] * (states[i+2][5] + states[i+2][6] + states[i+2][7]);
    }
}