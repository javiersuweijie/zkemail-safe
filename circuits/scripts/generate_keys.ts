...
const r1cPath = path.join(buildDir, "email_safe.r1cs");
...
// Output paths
const zKeyPath = path.join(buildDir, "email_safe.zkey");
...
async function exec() {
  ...
  await askBeacon();
  await zKey.beacon(path.join(buildDir, "circuit_1.zkey"), zKeyPath, "Final Beacon", ZKEY_BEACON, 10, console);
  log("✓ Beacon applied");

  await zKey.verifyFromR1cs(path.join(buildDir, "email_safe.r1cs"), phase1Path, zKeyPath, console);
